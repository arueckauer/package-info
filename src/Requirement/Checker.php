...
final class Checker
{
    ...
    public function checkRequirements(Head $head): array
    {
        ...
        foreach ($this->requirements as $requiredPackage => $lowestRequiredVersion) {
            $requirements[] = $this->checkRequirement($head, $requiredPackage, $lowestRequiredVersion);
        }
    ...
    public function checkDevelopmentRequirements(Head $head): array
    {
        ...
        foreach ($this->developmentRequirements as $requiredPackage => $lowestRequiredVersion) {
            $requirements[] = $this->checkDevelopmentRequirement($head, $requiredPackage, $lowestRequiredVersion);
        }

        return $requirements;
    }

    private function checkRequirement(Head $head, string $package, string $minimumVersion): Result
    {
        if (!$head->hasRequirement($package)) {
            return new Result($package, null, false, false);
        }
        ...
        return new Result($package, $versionConstraint, true, $isSupported);
    }

    private function checkDevelopmentRequirement(Head $head, string $package, string $minimumVersion): Result
    {
